## 1. Krok 1 — Předání mise

**Label:** Akce

**Nové artefakty na vstupu:**

- A_04_01 Zadání mise
- A_04_02 Protokol mise
- A_04_03 Mapa perimetru Centrály
- A_04_04 Terénní fotografie závalu

**Co potřebuje dítě k vyřešení kroku:**

- [Z BOXU MISE] A_04_01 Zadání mise
- [Z BOXU MISE] A_04_02 Protokol mise
- [Z BOXU MISE] A_04_03 Mapa perimetru Centrály
- [Z BOXU MISE] A_04_04 Terénní fotografie závalu
- V Zadání je vysvětleno: A měří pohyb u sopky, B u stanice, C na svahu; spojení přerušil zával, nikdo není uvězněný. V Protokolu dítě v poli „První domněnka“ označí sopku s otazníkem.

**Úkoly:**

- **U_04_01:** Projdi zadání, prohlédne mapu a fotografii závalu a založí Protokol mise.
- **U_04_02:** Pochopit situaci: Centrála zaznamenala neobvyklé otřesy v oblasti se spící sopkou.
- **U_04_03:** Pochopit původní úkol badatele: zjistit příčinu otřesů a ověřit bezpečnost perimetru.
- **U_04_04:** Zjistit, že během průzkumu došlo k závalu a přestal fungovat vzdálený přenos posledních měření z detašovaného pracoviště.
- **U_04_05:** Zapsat pracovní hypotézu: otřesy mohou souviset se sopkou.

**Co je hotový krok:**

- **U_04_01**
  - Prošel jsi podklady potřebné pro další úkol.
- **U_04_02**
  - Úkol je provedený podle zadání kroku.
- **U_04_03**
  - Úkol je provedený podle zadání kroku.
- **U_04_04**
  - Znáš odpověď a dokážeš ji doložit dostupnými podklady.
- **U_04_05**
  - Úkol je provedený podle zadání kroku.
- **Výsledek kroku**
  - Dítě zná incident, původní hypotézu i svůj cíl: získat chybějící měření a dokončit vyhodnocení bezpečnosti perimetru.

---
---
## 2. Krok 1_Report — Potvrzení přijetí mise

**Label:** Report

**Úkoly:**

- **U_04_06:** Přihlas se PAXovi.
- **U_04_07:** Potvrď převzetí případu.

**Co je hotový krok:**

- **U_04_06**
  - Hlášení nebo vysvětlení je předané.
- **U_04_07**
  - Hlášení nebo vysvětlení je předané.
- **Výsledek kroku**
  - PAX ví, že mise byla převzata.

---
---
## 3. Krok 2 — Přerušené spojení

**Label:** Akce

**Nové artefakty na vstupu:**

- A_04_05 Detašované odečtové pracoviště
- A_04_06 Označení ZÁVAL – NEVSTUPOVAT

**Co potřebuje dítě k vyřešení kroku:**

- [Z BOXU MISE] A_04_03 Mapa perimetru Centrály
- [Z BOXU MISE] A_04_04 Terénní fotografie závalu
- [Z BOXU MISE] A_04_05 Detašované odečtové pracoviště
- [Z BOXU MISE] A_04_06 Označení ZÁVAL – NEVSTUPOVAT
- A_04_01 vysvětluje lokální uložení dat. A_04_06 vyznačuje obdélník nepřístupné zóny. Stanice je uvnitř, ovládací místo vně. Dítě se před stavbou dozví: servisní vedení bylo natažené před závalem, dnes se k němu připojí nouzový modul.

**Úkoly:**

- **U_04_08:** Přesuň se do domácího „terénu“, najde označenou nebezpečnou oblast a na druhé straně vidí odečtové pracoviště. Porovnej situaci s mapou a fotografií.
- **U_04_09:** Ověřit, že pracoviště existuje a je odříznuté závalem.
- **U_04_10:** Zjistit, že se k němu nesmí vstoupit pěšky a poslední data jsou dostupná pouze lokálně na stanovišti.

**Co je hotový krok:**

- **U_04_08**
  - Předmět je na požadovaném místě.
- **U_04_09**
  - Kontrola je dokončená a její výsledek je určený.
- **U_04_10**
  - Znáš odpověď a dokážeš ji doložit dostupnými podklady.
- **Výsledek kroku**
  - Dítě má konkrétní problém: potřebuje získat data ze stanoviště, aniž vstoupí do nebezpečné zóny.

---
---
## 4. Krok 2_Report — Přerušený přístup

**Label:** Report

**Úkoly:**

- **U_04_11:** Nahlaš PAXovi situaci v terénu.

**Co je hotový krok:**

- **U_04_11**
  - Hlášení nebo vysvětlení je předané.
- **Výsledek kroku**
  - Centrála ví, že je nutné vytvořit jiné řešení.

---
---
## 5. Krok 3 — Nouzová inspekční lanovka

**Label:** Akce

**Nové artefakty na vstupu:**

- A_04_07 Konstrukční sada: dvě nízké základny s úchyty, předinstalované nosné vedení a ovládací smyčka, napínák u Centrály, obrázkový plán
- A_04_08 Servisní modul: skládací vozík se dvěma pojezdovými prvky a širokým čelním suchým zipem pro zachycení kazety

**Co potřebuje dítě k vyřešení kroku:** [Z BOXU OPERÁTORA] A_04_07–A_04_08 od PAXe, stanice A_04_05 s již instalovanou vzdálenou základnou a vedením, A_04_06 hranice. [Z DOMOVA] stůl nebo rovná plocha. Průvodce před začátkem upevní oba konce; dítě u Centrály sestaví vozík, nasadí ho na vedení, připojí ovládací smyčku a nastaví místní napínák.

**Úkoly:**

- **U_04_12:** Podle plánu sestaví modul a u bezpečného konce dokončí lanovou dráhu. Smyčkou pohybuje vozíkem vpřed i zpět. Při první jízdě zastaví před kazetou, vrátí modul a zkontroluje napnutí a průjezd.
- **U_04_13:** Sestrojit lanovku.
- **U_04_14:** Otestovat její průjezd nad závalem.
- **U_04_15:** Vyřešit případné zaseknutí nebo nevhodné napnutí.
- **U_04_16:** Dostat servisní modul bezpečně až k odečtovému pracovišti.

**Co je hotový krok:**

- **U_04_12**
  - Úkol je provedený podle zadání kroku.
- **U_04_13**
  - Úkol je provedený podle zadání kroku.
- **U_04_14**
  - Úkol je provedený podle zadání kroku.
- **U_04_15**
  - Úkol je provedený podle zadání kroku.
- **U_04_16**
  - Úkol je provedený podle zadání kroku.
- **Výsledek kroku**
  - Prázdný modul dojede ke značce před kazetou a vrátí se. Neklesne do zakázané oblasti ani se nezasekne. HERO stavba a test cílí na 15–20 minut. Vzdálené zaseknutí řeší dospělý po přerušení hry, dítě do zóny nevstupuje.

---
---
## 6. Krok 4 — Získání posledních dat

**Label:** Akce

**Nové artefakty na vstupu:** A_04_09 Datová kazeta s protikusem suchého zipu, získaná ze stanice; obsah zůstává zavřený do kroku 5

**Co potřebuje dítě k vyřešení kroku:** [Z BOXU MISE] otestovaná A_04_07, A_04_08 a A_04_05 stanice. Kazeta A_04_09 leží ve vodicím lůžku, protikus suchého zipu směrem k modulu. Široké čelo a doraz nastavují stejnou výšku; dosednutím se spojí, tahem zpět se kazeta vysune z lůžka.

**Úkoly:**

- **U_04_17:** Pošli servisní modul lanovkou ke stanovišti. Modul fyzicky zachytí datovou kazetu a dítě jej stáhne zpět.

**Co je hotový krok:**

- **U_04_17**
  - Úkol je provedený podle zadání kroku.
- **Výsledek kroku**
  - Kazeta je fyzicky u dítěte mimo zónu. Automatické zachycení má konkrétní návrh, ale jeho spolehlivost je NEOVĚŘENO; před dětským průchodem je nutný prototyp.

---
---
## 7. Krok 4_Report — Data obnovena

**Label:** Report

**Úkoly:**

- **U_04_18:** Nahlaš PAXovi, že data získalo.

**Co je hotový krok:**

- **U_04_18**
  - Hlášení nebo vysvětlení je předané.
- **Výsledek kroku**
  - Centrála může pokračovat ve vyhodnocení.

---
---
## 8. Krok 5 — Vyhodnocení měření

**Label:** Akce

**Nové artefakty na vstupu:** A_04_10 Složená karta měření A–B–C uvnitř kazety A_04_09, zpřístupněná jejím otevřením

**Co potřebuje dítě k vyřešení kroku:** [Z BOXU MISE] A_04_09, uvnitř A_04_10, A_04_02 pole „Měření před závalem“, značicí prostředek. Karta výslovně uvádí tři po sobě jdoucí odečty PŘED závalem: A 1–1–1, B 1–1–1, C 1–2–3. Tečky znázorňují modelovou úroveň pohybu, ne skutečné fyzikální jednotky. Legenda: rostoucí řada = místo k uzavření a odborné kontrole.

**Úkoly:**

- **U_04_19:** Otevři kazetu a vyjme záznam. S dospělým přečte symboly A–B–C a pořadí odečtů. V předtištěných třech řadách Protokolu vybarví tolik polí, kolik ukazují tečky, a porovná průběhy.
- **U_04_20:** Doplnit jednotlivá měření.
- **U_04_21:** Porovnat průběh tří čidel.
- **U_04_22:** Najít čidlo, jehož hodnoty postupně rostou.

**Co je hotový krok:**

- **U_04_19**
  - Úkol je provedený podle zadání kroku.
- **U_04_20**
  - Úkol je provedený podle zadání kroku.
- **U_04_21**
  - Úkol je provedený podle zadání kroku.
- **U_04_22**
  - Úkol je provedený podle zadání kroku.
- **Výsledek kroku**
  - Dítě prokáže, že pouze hodnoty čidla C soustavně rostly.

---
---
## 9. Krok 6 — Zdroj nebezpečí

**Label:** Akce

**Nové artefakty na vstupu:** A_04_11 Značka NESTABILNÍ OBLAST

**Co potřebuje dítě k vyřešení kroku:** [Z BOXU MISE] A_04_02 vyplněný graf, A_04_10 časové zařazení a legenda, A_04_03 mapa se sopkou A, stanicí B a svahem C, A_04_11 značka. Časy před závalem vylučují, že šlo pouze o měření samotného pádu kamenů.

**Úkoly:**

- **U_04_23:** Najdi na mapě čidla A, B a C a propojí jejich polohu s výsledkem grafu. Označ oblast u C. V poli „Doporučení“ označí svah a odbornou kontrolu. Neoznačuje sopku za definitivně bezpečnou ani neprohlašuje příčinu otřesů za prokázanou.
- **U_04_24:** Zjistit, kde čidlo C leží.
- **U_04_25:** Porovnat jeho polohu se sopkou.
- **U_04_26:** Označit rizikový skalní svah na mapě.
- **U_04_27:** Zapsat závěr do Protokolu M!SE.

**Co je hotový krok:**

- **U_04_23**
  - Hledané místo nebo předmět jsi našel.
- **U_04_24**
  - Znáš odpověď a dokážeš ji doložit dostupnými podklady.
- **U_04_25**
  - Úkol je provedený podle zadání kroku.
- **U_04_26**
  - Požadované označení nebo záznam je provedený.
- **U_04_27**
  - Úkol je provedený podle zadání kroku.
- **Výsledek kroku**
  - Dítě zjistí: A u sopky nevykazuje růst, zatímco C na skalním svahu ano. Data tedy nepodporují původní hypotézu o sopce a ukazují na nestabilitu svahu.

---
---
## 10. Krok 6_Report — Výsledek průzkumu

**Label:** Report

**Úkoly:**

- **U_04_28:** Nahlaš PAXovi, které čidlo vykazovalo rostoucí aktivitu a kde leží.

**Co je hotový krok:**

- **U_04_28**
  - Hlášení nebo vysvětlení je předané.
- **Výsledek kroku**
  - Centrála získává podklad k uzavření nebezpečné části perimetru a dalšímu odbornému prověření svahu.

---
---
## 11. Krok 7 — Zajištění perimetru

**Label:** Akce

**Nové artefakty na vstupu:** A_04_12 Značka / pečeť ZÓNA UZAVŘENA

**Co potřebuje dítě k vyřešení kroku:**

- [Z BOXU MISE] A_04_03 Mapa perimetru Centrály
- [Z BOXU MISE] A_04_12 Značka / pečeť ZÓNA UZAVŘENA
- Pokyn PAXe po reportu 6 uzavřít označený svah; A_04_02 pole „Zóna uzavřena“.

**Úkoly:**

- **U_04_29:** Označ svah C pečetí uzavření, zkontroluje shodu s Protokolem a fyzicky ukáže nebo předá mapu průvodci na místě Centrály.

**Co je hotový krok:**

- **U_04_29**
  - Požadované označení nebo záznam je provedený.
- **Výsledek kroku**
  - Mapa perimetru je aktualizovaná a Centrála ví, kterou část území nemá používat.

---
---
## 12. Krok 8 — Uzavření M!SE

**Label:** Akce

**Nové artefakty na vstupu:** A_04_13 Badge KONSTRUKTÉR

**Co potřebuje dítě k vyřešení kroku:**

- [Z BOXU MISE] A_04_02 Protokol mise
- [Z BOXU MISE] A_04_13 Badge KONSTRUKTÉR

**Úkoly:**

- **U_04_30:** Dokonči Protokol mise a převezme specializaci.

**Co je hotový krok:**

- **U_04_30**
  - Výsledek je sestavený podle zadání kroku.
- **Výsledek kroku**
  - Centrála má mapu uzavřeného svahu a zachráněná data. Dítě dokončilo Protokol, získalo badge KONSTRUKTÉR a ponechá si servisní modul a sestavitelnou lanovku.
